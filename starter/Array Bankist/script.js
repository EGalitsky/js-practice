...
const checkDogs = function (arr, arr2) {
  const dogsJuliaCorrect = arr.slice(1, -2);
  const dogs = dogsJuliaCorrect.concat(arr2);
...
*/
/*
const calcAverageHumanAgePipe = ages =>
  ages
    .map(age => (age <= 2 ? 2 * age : 16 + age * 4))
    .filter(age => age >= 18)
    .reduce((acc, age, i, arr) => acc + age / arr.length, 0);

const avg1 = calcAverageHumanAgePipe([5, 2, 4, 1, 15, 8, 3]);
const avg2 = calcAverageHumanAgePipe([16, 6, 10, 5, 6, 1, 4]);
console.log(avg1);
console.log(avg2);
*/
/*
...
const accountForOf = function (accts) {
...
const dogs = [
  { weight: 32, curFood: 250, owners: ['Alice', 'Bob'] },
...
// console.log(dogs);
//2.
function eating(dogs, owner) {
  dogs.forEach(dog => {
    if (dog.owners.includes(`${owner}`))
      console.log(
        `The ${owner}'s dog is eating ${
          dog.curFood > dog.recommendedFood ? 'too much' : 'too little'
        }`
      );
  });
}
